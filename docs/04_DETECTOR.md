...
## Archivo: `app/chat/nl2sql/detector.py`

```python
"""Query detector - determines if a message requires database access."""
import json
import logging
import re
...
class QueryDetector:
    """Detects if a user message requires a database query."""
    ...
    def _heuristic_check(self, message: str) -> Tuple[bool, float, str]:
        """Fast heuristic check based on keywords."""
        ...
        if data_score == 0 and chat_score == 0:
            return (False, 0.3, "No keywords detected")
        ...
        reasoning = f"Heuristic: {data_score} data keywords, {chat_score} chat keywords"
        return (is_data, confidence, reasoning)

    async def _llm_detection(self, message: str) -> Tuple[bool, float, str]:
        """Use Gemini for more accurate detection."""
